...
1. Списък (list)
    ...
    - List comprehension:
      ```py
      [x**2 for x in range(10) if x % 2 == 0]  # Само квадратите на четните числа от 0 до 9 вкл. → [0, 4, 16, 36, 64]
      ```
...
2. Кортеж (tuple)
    ...
    - Имат само методите count() и index()
      ...
      ```
    - НЕ поддържат методи за промяна на съдържанието (append(), remove(), sort() и т.н.).
